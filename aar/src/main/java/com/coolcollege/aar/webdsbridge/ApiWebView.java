...
public class ApiWebView extends WebView {
    ...
    class InnerJavascriptInterface {
        ...
        public String call(String methodName, String argStr) {
            ...
            Log.i("WebView", "methodName:" + methodName);
...
//                                        script += "delete window." + cb;
    ...
    public interface OnLoadFinishListener {
        void onLoadFinished(int progress, String url);
    }
    ...
    public void removeLoadFinishListener() {
    ...
    private OnLoadProcessListener onLoadProcessListener;

    public void setOnLoadProcessListener(OnLoadProcessListener onLoadProcessListener) {
        this.onLoadProcessListener = onLoadProcessListener;
    }

    public void removeOnLoadProcessListener() {
        onLoadProcessListener = null;
    }
    ...
    public interface WebFileChooserListener {
        ...
        boolean onShowFileChooser(WebView mWebView, ValueCallback<Uri[]> filePathCallback, WebChromeClient.FileChooserParams fileChooserParams);

        void openFileChooser(ValueCallback<Uri> uploadMsg, String acceptType, String capture);
    }

    private WebFileChooserListener webFileChooserListener;

    public void setWebFileChooserListener(WebFileChooserListener webFileChooserListener) {
        this.webFileChooserListener = webFileChooserListener;
    }

    public void removeWebFileChooserListener() {
        webFileChooserListener = null;
    }
    ...
    private void init() {
        ...
        settings.setSupportZoom(true); // 是否支持缩放，默认值为true
        settings.setBuiltInZoomControls(true); // 是否使用内置的缩放机制，默认值为false
...
//                EventBus.getDefault().post(new MessageModel<String>(MESSAGE_TYPE_SHOW_LEFT_VIEW, ""));
...
//                EventBus.getDefault().post(new MessageModel<>(MESSAGE_TYPE_SHOW_LEFT_VIEW, ""));
                if (webView.getUrl().startsWith("tel:")) {
                    try {
                        webView.getContext().startActivity(new Intent(Intent.ACTION_DIAL, Uri.parse(webView.getUrl())));
                    } catch (Exception e) {
                    }
                    return true;
                }
...
//                ShowLeftView(view, url);
            }
        });
    }

    /* 展示回退按钮的操作 */
    private void ShowLeftView(WebView webView, String s) {
//        EventBus.getDefault().post(new MessageModel<String>(MESSAGE_TYPE_SHOW_LEFT_VIEW, ""));
        if (onLoadFinishListener != null) {
            onLoadFinishListener.onLoadFinished(webView.getProgress(), s);
        }
    }
    ...
    private void addInternalJavascriptObject() {
        addJavascriptObject(new Object() {
            ...
            public void dsinit(Object jsonObject) {
                ApiWebView.this.dispatchStartupQueue();
            }
    ...
    private void _evaluateJavascript(String script) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            ApiWebView.super.evaluateJavascript(script, null);
        } else {
    ...
    public void loadUrl(final String url, final Map<String, String> additionalHttpHeaders) {
        runOnMainThread(new Runnable() {
            ...
            public void run() {
                callInfoList = new ArrayList<>();
                ApiWebView.super.loadUrl(url, additionalHttpHeaders);
            }
        });
    }
    ...
    private WebChromeClient mWebChromeClient = new WebChromeClient() {
        ...
        public void onProgressChanged(WebView view, int newProgress) {
            if (onLoadProcessListener != null) {
                onLoadProcessListener.onProcessChanged(newProgress);
            }
        ...
        public boolean onShowFileChooser(WebView webView, ValueCallback<Uri[]> filePathCallback,
            ...
            if (webFileChooserListener != null) {
                return webFileChooserListener.onShowFileChooser(webView, filePathCallback, fileChooserParams);
            }
        ...
        public void openFileChooser(ValueCallback valueCallback, String acceptType) {
            if (webFileChooserListener != null) {
                webFileChooserListener.openFileChooser(valueCallback, acceptType);
            }
        ...
        public void onPermissionRequest(PermissionRequest request) {
            mRequest = request;
            for (String req : request.getResources()) {
                switch (req) {
                    case REQ_AUDIO_CAPTURE:
    ...
    private void requestAudioPermission() {
        PermissionManager.checkPermissions((Activity) getContext(), new PermissionStateListener() {
            @Override
