...
import com.coolcollege.aar.module.NotchInfoModel;
...
public class TitleBar extends FrameLayout
        ...
        switch (MeasureSpec.getMode(heightMeasureSpec)) {
            ...
            case MeasureSpec.UNSPECIFIED:
                ...
                if (background instanceof BitmapDrawable) {
                    mMainLayout.getLayoutParams().height = sDefaultStyle.getTitleBarHeight() + (notchInfo != null ? notchInfo.getHeight() : BarUtils.getStatusBarHeight());
                    // 算出标题栏的宽度和图片的宽度之比例
                    ...
                    // 获取 TitleBar 默认高度
                    heightMeasureSpec = MeasureSpec.makeMeasureSpec(sDefaultStyle.getTitleBarHeight() + (notchInfo != null ? notchInfo.getHeight() : BarUtils.getStatusBarHeight()), MeasureSpec.EXACTLY);
                }
                break;
            case MeasureSpec.EXACTLY:
            default:
    ...
    NotchInfoModel notchInfo;

    public void setNotchInfo(NotchInfoModel notchInfo) {
        this.notchInfo = notchInfo;
        if (notchInfo != null && notchInfo.isHasNotch())
            setStatusHeight(notchInfo.getHeight());
        else {
            setStatusHeight(BarUtils.getStatusBarHeight());
        }
    }

    public void setStatusHeight(int height) {
        statusView.setLayoutParams(new LinearLayout.LayoutParams(LayoutParams.MATCH_PARENT, height));
    }

    private void initView(Context context) {

        mMainLayout = ViewBuilder.newMainLayout(context);
        mTitleLayout = ViewBuilder.newTitleLayout(context);
        ...
        if (notchInfo == null) {
            statusView = ViewBuilder.newStatusBarView(context, 0);
        } else {
            statusView = ViewBuilder.newStatusBarView(context, notchInfo.getWidth());
        }
    ...
    private void initBar() {
        mTitleLayout.addView(mLeftView);
        mTitleLayout.addView(mTitleView);
        mTitleLayout.addView(mRightView);


        mMainLayout.addView(statusView);
        mMainLayout.addView(mTitleLayout);
    ...
    public void releaseListener() {
        mTitleView.removeCallbacks(this);
        mLeftView.removeCallbacks(this);
        mRightView.removeCallbacks(this);
    ...
    public void setmListener(OnTitleBarListener mListener) {
    ...
    public TitleBar setRightTitle(@StringRes int id) {
        return setRightTitle(getResources().getString(id), true);
    }

    public TitleBar setRightTitle(CharSequence text, boolean show) {
    ...
    public void isRightViewShow(boolean isShow) {
        if (isShow) {
